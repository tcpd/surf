...
public class StreakMergeAlgorithm extends MergeAlgorithm {
    ...
    public List<Collection<Row>> run() {
        ...
        for (String id: new LinkedHashSet<>(idToRows.keys())) {
            ...
            Set<Integer> holeValues = new LinkedHashSet<>(); // this will have the holes in the streak field
            ...
            Set<String> secondaryValsForThisId = rowsForThisID.stream().map(r -> r.get(secondaryFieldName)).collect(Collectors.toSet());

            log.info("Looking for streak candidates for id: " + id + " with " + rowsForThisID.size() + " rows, "
                    + holeValues.size() + " hole(s): " + Util.join (holeValues, ",")
                    + ", " + secondaryValsForThisId.size() + " secondary value(s): " + Util.join(secondaryValsForThisId, ","));
            log.info (Util.join (rowsForThisID.stream().map(r -> r.get(Config.MERGE_FIELD)).collect(Collectors.toSet()), ","));
            ...
            for (int holeValue: holeValues) {
                ...
                Collection<Row> holeCandidates = rowsInHole.stream().filter(r -> secondaryValsForThisId.contains(r.get(secondaryFieldName))).collect(Collectors.toSet());

                // for every hole candidate row, create a pair with that row and it's distance from streak rows
                holeCandidateAndDistance.addAll (holeCandidates.stream().map(r -> rowToStreakRowsDistance(r, rowsForThisID)).collect(Collectors.toList()));
            ...
            // but we actually want decreasing distance
            Collections.reverse(holeCandidateAndDistance);
            // now extract only the rows
            List<Row> holeCandidateRows = holeCandidateAndDistance.stream().map (Pair::getFirst).collect(Collectors.toList());
